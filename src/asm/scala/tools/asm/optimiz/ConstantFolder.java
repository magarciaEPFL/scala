...
public class ConstantFolder implements Opcodes {
            ...
            if (insn != null) {
                ...
                switch (opc) {
                    ...
                    case Opcodes.LREM:
                        ...
                        /*
                         * Special care is required not to swallow a j.l.ArithmeticException
                         * that integer division (as well as remainder) throws for zero divisor,
                         * ie for zero divisor the opcodes IDIV, LDIV, IREM, and LREM throw that exception.
                         *
                         * On the other hand, similar opcodes for floating-precision won't throw any exception,
                         * thus constant folding can be done for them.
                         *
                         * In all cases, constant folding is always performed provided no j.l.ArithmeticException can happen.
                         * This can be seen in the overrides of `CFValue.div(CFValue)` and `CFValue.rem(CFValue)`
                         * which return `Unknown` for integer (division or remainder) with zero divisor.
                         *
                         * Moreover, unlike for other binary operations, for integer division and remainder it's not desirable to:
                         *   - leave the operation in place,
                         *   - insert a DROP right after it
                         * so that PushPopCollapser cancels out the DROP with the (IDIV or LDIV or IREM or LREM)
                         * because in general that's not safe given the information PushPopCollapser has at its disposal.
                         *
                         * Instead, the rewriting for (IDIV or LDIV or IREM or LREM) replaces that opcode
                         * with two DROPs, followed by LDC of the resulting constant.
                         *
                         */

                        if (nxtFrame.getStackTop().isConstant()) {
                            dropArgsLoadResult(insn, (Constant)nxtFrame.getStackTop());
                            changed = true; // actually not needed: control-flow unaltered, no code has been killed.
                        }
                        break;
                    ...
                    case Opcodes.LCMP:
                        ...
                        if (nxtFrame.getStackTop().isConstant()) {
                            dropResultLoadResult(insn, (Constant)nxtFrame.getStackTop());
                            changed = true; // actually not needed: control-flow unaltered, no code has been killed.
                        }
                        break;
    ...
    private void dropArgsLoadResult(final AbstractInsnNode insn, Constant cst) {
        mnode.instructions.insert(insn, cst.pushInsn());
        int sz = cst.getSize();
        mnode.instructions.insert(insn, Util.getDrop(sz));
        mnode.instructions.insert(insn, Util.getDrop(sz));
        mnode.instructions.remove(insn);
    }

    private void dropResultLoadResult(final AbstractInsnNode insn, Constant cst) {
        mnode.instructions.insert(insn, cst.pushInsn());
    ...
    static private abstract class CFValue implements Value {
        ...
        public abstract CFValue rem(CFValue value2);
    ...
    final static private class Unknown extends CFValue {
        ...
        @Override public Unknown rem(CFValue value2) { assert size == value2.size; return this; }
    ...
    final static private class ICst extends Constant {
         ...
         */
        @Override public CFValue rem(CFValue value2) {
            assert size == value2.size;
            if (value2.isUnknown()) {
                return value2;
            } else {
    ...
    final static private class FCst extends Constant {
        ...
        @Override public boolean isNaN() { return java.lang.Float.isNaN(v); }
        ...
        /**
         * Quoting from the JVMS:
         *   Despite the fact that division by zero may occur, evaluation of an frem
         *   instruction never throws a runtime exception. Overflow, underflow, or
    ...
    final static private class JCst extends Constant {
        ...
        @Override public CFValue rem(CFValue value2) {
            ...
            if (value2.isUnknown()) {
                ...
                JCst that = (JCst) value2;
                if (that.v == 0) { return Unknown.UNKNOWN_2; }
                else { return new JCst(v % that.v); }
            }
        }
    ...
    static private class DCst extends Constant {
        ...
        @Override public boolean isNaN() { return java.lang.Double.isNaN(v); }
        ...
        /**
         * Quoting from the JVMS:
         *   Despite the fact that division by zero may occur, evaluation of an drem
         *   instruction never throws a runtime exception. Overflow, underflow, or
         *   loss of precision cannot occur.
         */
        @Override public CFValue rem(CFValue value2) {
    ...
    static public class ConstantInterpreter extends Interpreter<CFValue> {
        ...
        public CFValue merge(final CFValue d, final CFValue w) {
            ...
            if (
                    (d.isConstant() && ((Constant)d).isNaN()) ||
                    (w.isConstant() && ((Constant)w).isNaN())
               ) {
