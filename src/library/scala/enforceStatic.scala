 ...
 *
 * The compiler accepts @enforceStatic occurrences provided the target module-class
 ...
 * the singleton in question. However, @enforceStatic results in all callsites
 ...
 *
 * Finally, an object annotated with @enforceStatic that also defines @inline methods
 ...
 *
 * The caveats above are listed for completeness. When used for its intended purpose, @enforceStatic is really useful!
 *
 * Before @enforceStatic, functionality defined on objects (and in the companion to value-classes) was accessed following the idiom:
 ...
 * This is what the @enforceStatic annotation allows,
 ...
 *       but extending an interface that declares one or more members results in an error for an @enforceStatic-tagged class.
 *
 *
 *  @author  Miguel Garcia, http://lampwww.epfl.ch/~magarcia/ScalaCompilerCornerReloaded/
 *  @since   2.11
 */
class enforceStatic extends scala.annotation.StaticAnnotation
