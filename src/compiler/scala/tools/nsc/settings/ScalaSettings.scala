...
trait ScalaSettings extends AbsScalaSettings
  ...
  val nooptimise    = BooleanSetting("-Ynooptimise", "Clears all the flags set by -optimise. Useful for testing optimizations in isolation.") withAbbreviation "-Ynooptimize" disabling optimise::Nil
   ...
   *
   *    case 0 => Just emit trees as delivered by CleanUp.
   *
   ...
   *
   */
  private def neoLevel: Int   = (
    if (Ybackend.value.startsWith("o")) Ybackend.value.substring(1).toInt
    else if (optimise.value) 3
    else 0
  )
  ...
  def isClosureConvDelegating  = if (closureConv.isSetByUser) closureConv.value == "delegating" else true
