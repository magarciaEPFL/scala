...
abstract class Constructors extends Transform with ast.TreeDSL {
  ...
  class ConstructorTransformer(unit: CompilationUnit) extends Transformer {
    ...
    override def transform(tree: Tree): Tree = {
      tree match {
        case cdef @ ClassDef(mods0, name0, tparams0, impl0) if !cdef.symbol.isInterface && !isPrimitiveValueClass(cdef.symbol) =>

          val hasPrimaryConstructor = { impl0.body exists (_.symbol.isPrimaryConstructor) }

          if(hasPrimaryConstructor) {
            val tplTransformer = new TemplateTransformer(unit, impl0)
            treeCopy.ClassDef(cdef, mods0, name0, tparams0, tplTransformer.transformed)
          } else {
            // AnyVal constructor is OK
            val clazz = impl0.symbol.owner  // the transformed class
            assert(clazz eq AnyValClass, "no constructor in template: impl = " + impl0)
            cdef
          }
  ...
  class TemplateTransformer(unit: CompilationUnit, val impl: Template) extends Transformer with DelayedInitHelper {
    ...
    val constrInfo: ConstrInfo = {
      stats find (_.symbol.isPrimaryConstructor) match {
        ...
        case x =>
          unit.error(impl.pos, "Shouldn't have got here, TemplateTransformer found no constructor for = " + impl)
