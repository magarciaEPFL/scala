...
// ------ Transforming anonymous functions and by-name-arguments ----------------
    ...
    def closureConversionModern(fun: Function): Tree = {
       ...
       *  There a risk that the return type of hoisted will already be AbstractFunctionN,
       *  and the asInstanceOf will be treated as redundant and eliminated,
       *  and consequently the tree won't be recognized as a late-closure by the bytecode emitter.
       *  Solution sketch: a compiler-only synthetic method (thus used by nobody else) with the same signature of asInstanceOf.
       *  The thus wrapped "fake callsite" would then look like:
       *    myAsInstanceOf[AbstractFunctionN[T_1, .., T_N, R]](hoisted(zeroes-for-params-above))
       */
