 ...
 *
 */
abstract class BCodeTFA extends BCodeHelpers with BytecodeWriters {
  ...
  case class TFValue(lca: BType, bits: Int) extends asm.tree.analysis.Value {
    ...
    val tr: Tracked = {
      if (lca.isPhantomType || lca.isValueType) { null }
        ...
        assert(lca.isNonSpecial, s"Some case was overlooked for: $lca")
        exemplars.get(lca)
      }
    }

    // check (when possible) cross-consistency with isExact and isNonNull
    assert(if (lca.isNullType) !isNonNull else true, s"Contradiction: NullType reported as isNonNull: $lca")
    assert(if (tr != null && tr.isFinal)      isExact    else true, s"Contradiction: isFinal reported as !isExact: $lca")
   ...
   **/
  class TypeFlowInterpreter extends backend.bcode.InterpreterSkeleton[TFValue] {
    ...
    override def merge(v: TFValue, w: TFValue): TFValue = {
      ...
      val lub: BType = {
        ...
        if      (a.isValueType)   { maxValueType(a, b)   }
          ...
          assert(a.isRefOrArrayType, s"This is not a valuetype and it's not something else, what is it? $a")
          assert(b.isRefOrArrayType, s"Attempting to merge $b with $a")
          ...
          if (a.isNullType)      { b }
            ...
            if (a.isArray || b.isArray) { arrayLUB(a, b) }
              ...
              assert(a.hasObjectSort, s"Expecting non-array referene, received: $a")
              assert(b.hasObjectSort, s"Expecting non-array referene, received: $b")
      ...
      if (xArr && yArr) {
        ...
        if (xCT == yCT) { x }
          ...
          if      (xCT.isValueType || yCT.isValueType) { ObjectReference }
            ...
            assert(xCT.hasObjectSort, s"Expecting non-array referene, received: $xCT")
            assert(yCT.hasObjectSort, s"Expecting non-array referene, received: $yCT")
    ...
    override def opANEWARRAY(insn: asm.tree.TypeInsnNode): TFValue = {
      ...
      assert(c != '(', s"Unexpected method type: ${insn.desc}")
      val arrType =
        if (c == '[') { "["  + insn.desc }
        else          { "[L" + insn.desc + ";" }
