...
abstract class BCodeBuilder extends BCodeTypes {
  import global._
   ...
   *
   */
  abstract class PlainBaseBuilder(cunit: CompilationUnit)
    extends BCClassGen
    with    BCAnnotGen
    ...
    object bc extends JCodeMethodN {
      override def jmethod = PlainBaseBuilder.this.mnode
    }
    ...
    val locals = mutable.Map.empty[Symbol, Local] // (local-or-param-sym -> Local(TypeKind, name, idx))
    var nxtIdx = -1 // next available index for local-var
