  ...
  abstract class PlainBodyBuilder(cunit: CompilationUnit) extends LateClosureBuilder(cunit) {
    ...
    def genArithmeticOp(tree: Tree, code: Int): BType = {
      ...
      assert(resKind.isNumericType || (resKind == BOOL),
             s"${resKind.getDescriptor} is not a numeric or boolean type [operation: ${fun.symbol}]")
        ...
        case rarg :: Nil =>
          ...
          if (scalaPrimitives.isShiftOp(code) || scalaPrimitives.isBitwiseOp(code)) {
            assert(resKind.isIntegralType || (resKind == BOOL),
                   s"${resKind.getDescriptor} incompatible with arithmetic modulo operation.")
          }
          ...
          generatedType match {
            ...
            case rt if generatedType.hasObjectSort =>
              ...
              if (!bes.isEmpty) {
                ...
                rephraseBackedgesInCtorArg(bes, cnode, mnode, newInsn, initInsn)
              }
     ...
     *      INVOKESPECIAL <init>
     *
     *  TODO Update: As of Oracle's Java SE 1.7.0_15-b03, the bug doesn't show up anymore. Apparently it's confined to early releases of JDK 7.
     *
     ...
     *
     */
    def rephraseBackedgesInCtorArg(bes:      _root_.java.util.Map[asm.tree.JumpInsnNode, asm.tree.LabelNode],
                                   cnode:    asm.tree.ClassNode,
                                   mnode:    asm.tree.MethodNode,
                                   newInsn:  asm.tree.TypeInsnNode,
                                   initInsn: asm.tree.MethodInsnNode) {


      def methodSignature(cnode: asm.tree.ClassNode, mnode: asm.tree.MethodNode): String = {
        cnode.name + "::" + mnode.name + mnode.desc
      }

      def insnPos(insn: asm.tree.AbstractInsnNode, mnode: asm.tree.MethodNode): String = {
        s"${asm.optimiz.Util.textify(insn)} at index ${mnode.instructions.indexOf(insn)}"
      }

      import scala.collection.JavaConverters._
      for(
        entry <- bes.entrySet().asScala;
        jump  = entry.getKey;
        label = entry.getValue
      ) {
        warning(
          sm"""Backedge found in contructor-args section, in method ${methodSignature(cnode, mnode)}
              | (jump ${insnPos(jump, mnode)} , target ${insnPos(label, mnode)} ).
              | In order to avoid SI-6720, adding LOADs and STOREs for arguments.
              | As a result, ${newInsn.desc} is now instantiated after evaluating all ctor-arguments,
              | on the assumption such class has not static-ctor performing visible side-effects that could make a difference."""
        )
      }
      ...
      for(i <- paramTypes.indices.reverse) {
        val pt = paramTypes(i)
        val Local(_, _, idxVar, _) = locals(locals.makeLocal(pt, "cachedCtorArg"))
