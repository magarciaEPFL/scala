...
abstract class GenBCode extends BCodeOptMethodHandles {
  ...
  class BCodePhase(prev: Phase) extends StdPhase(prev) {
    ...
    class Worker1(needsOutFolder: Boolean) extends _root_.java.lang.Runnable {
      ...
      val mustPopulateCodeRepo  = isInliningRun || isDebugRun || settings.isClosureConvMH
    ...
    class Worker2 extends _root_.java.lang.Runnable {
      ...
      def visit(item: Item2) {
        ...
        if(item.dClosureEndpoints != null) {
          populateDClosureMaps(cnode, cnodeBT, item.dClosureEndpoints)
        }
