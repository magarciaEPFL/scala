  ...
  final class DClosureOptimizerImpl(masterCNode: ClassNode) {
    ...
    def treeShakeUnusedDClosures(): Boolean = {
      ...
      for(d <- liveDCs) {
        ...
        val unused = (masterCNode.toMethodList forall { mnode => closuRepo.closureAccesses(mnode, d).isEmpty })
        if (unused) {
          changed = true
    ...
    private def minimizeDClosureFields(endpoint: MethodNode, d: BType): Boolean = {
      ...
      def adaptEndpointAndItsCallsite(): Boolean = {
        ...
        if (!elidedParams.isEmpty) {
          ...
          for(dmethod <- dCNode.toMethodList) {
      ...
      for (caller <- dCNode.toMethodList) {
      ...
      val closureState: Map[String, FieldNode] = {
        dCNode.toFieldList filter { fnode => Util.isInstanceField(fnode) } map { fnode => (fnode.name -> fnode) }
      }.toMap
      val whatGetsRead = mutable.Set.empty[String]
      for(
        caller <- dCNode.toMethodList;
      ...
      val ctor = (dCNode.toMethodList find { caller => caller.name == "<init>" }).get
    ...
    private def singletonizeDClosures() {
      for(d <- closuRepo.liveDClosures(masterCNode)) {
        ...
        if (lacksClosureState) {
          ...
          staticClassInitializer.visitTypeInsn(Opcodes.NEW, dCNode.name)
          staticClassInitializer.visitInsn(Opcodes.DUP)
          staticClassInitializer.visitMethodInsn(Opcodes.INVOKESPECIAL, dCNode.name, "<init>", "()V")
          staticClassInitializer.visitFieldInsn(Opcodes.PUTSTATIC, dCNode.name, single.name, dClassDescriptor)
          ...
          for(
            callerInMaster <- masterCNode.toMethodList;
