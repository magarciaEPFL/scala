...
abstract class BCodeOptIntra extends BCodeSyncAndTry {
  ...
  class EssentialCleanser(cnode: asm.tree.ClassNode) {
    ...
    val jumpsCollapser      = new backend.asm.JumpChainsCollapser()
    val unreachCodeRemover  = new backend.asm.UnreachableCode()
    val labelsCleanup       = new backend.asm.LabelsCleanup()
    val danglingExcHandlers = new backend.asm.DanglingExcHandlers()
