  ...
  trait BCInnerClassGen {
    ...
    final def toTypeKind(t: Type): BType = {
      ...
      t.normalize match {
        ...
        case _: ExistentialType     => abort(s"ExistentialType reached GenBCode: $t")
        case AnnotatedType(_, w, _) => toTypeKind(w) // TODO test/files/jvm/annotations.scala causes an AnnotatedType to reach here.
        case _: RefinedType         => abort(s"RefinedType reached GenBCode: $t") // defensive: parents map toTypeKind reduceLeft lub
