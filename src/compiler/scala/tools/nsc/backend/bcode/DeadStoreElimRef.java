...
public class DeadStoreElimRef {
    ...
    private DisambiguatingAnalyzer propag = null;
    ...
    public void transform(final String owner, final MethodNode mnode) throws AnalyzerException {
        ...
        propag = DisambiguatingAnalyzer.create();
    ...
    private boolean isNULL(final SourceValue sv) {
        ...
        for (AbstractInsnNode prod : sv.insns) {
            if (prod instanceof DisambiguatingAnalyzer.FakeInsn) {
                return false;
            }
    ...
    private boolean isTHIS(final SourceValue sv) {
        assert !(sv.insns.isEmpty());
        for (AbstractInsnNode prod : sv.insns) {
            if (!(prod instanceof DisambiguatingAnalyzer.FakeParamLoad)) {
                return false;
            }
            final DisambiguatingAnalyzer.FakeParamLoad fp = (DisambiguatingAnalyzer.FakeParamLoad) prod;
            if (!fp.isThisRef()) {
                return false;
            }
        }
    ...
    private boolean isUninit(final SourceValue sv) {
        assert !(sv.insns.isEmpty());
        for (AbstractInsnNode prod : sv.insns) {
            if (!(prod instanceof DisambiguatingAnalyzer.Uninitialized)) {
                return false;
            }
        }
        return true;
