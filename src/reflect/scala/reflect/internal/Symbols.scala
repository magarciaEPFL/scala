...
// -------------- Symbol-savvy collections ------------------------------------------
   ...
   *  a Set[Int] (containing symbol-ids) could be used to the same effect
  ...
  final class SetOfSymIds {
    ...
    private var keytable: Array[Int] = newEmptyArray(SetOfSymIds.initSize)
    private var sz = 0

    private def newEmptyArray(size: Int) = new Array[Int](size)

    private def insert(k: Int) {
      checkResize()
    ...
    def ++=(syms: Iterable[Symbol]) {
      syms foreach +=
    }

    def ++=(that: SetOfSymIds) {
      for(k <- that.keytable; if k > 0) { insert(k) }
    }
    ...
    def clear() {
      keytable = newEmptyArray(SetOfSymIds.initSize)
      sz = 0
    }
  ...
  object SetOfSymIds {
    ...
    def create(that: SetOfSymIds): SetOfSymIds = {
      val s = new SetOfSymIds
      s.keytable = that.keytable.clone()
      s.sz = that.sz;
      s
    }
